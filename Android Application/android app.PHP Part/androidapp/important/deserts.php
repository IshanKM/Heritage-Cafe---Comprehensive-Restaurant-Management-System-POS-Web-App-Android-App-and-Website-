<?php 
	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST') 
	 {
	 	$qury = "SELECT COUNT(id) FROM `items` WHERE Category = 'deserts' OR Category = 'drink' AND Availability = 'Available'";
	    $result = mysqli_query($connect,$qury);
	    $row = mysqli_fetch_array($result);
	    $response['size'] = $row['COUNT(id)'];

	   $qury = "SELECT * FROM `items` WHERE Category = 'deserts' OR Category = 'drink' AND Availability = 'Available' ORDER BY Price ASC";
	    $result = mysqli_query($connect,$qury);
	    $i = 0;
